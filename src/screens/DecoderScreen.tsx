...
export function DecoderScreen() {
  ...
  const decodeFromLocalUri = useCallback(
    async (uri: string) => {
      try {
        setLoading(true);
        setError('');
  ...
  const handlePasteFromClipboard = useCallback(async () => {
    try {
      ...
      if (
        ...
        setError(
          'Clipboard does not contain an image, image URL, or base64 image data',
        );
      }
    } catch (err: any) {
  ...
  const handleOpenUrl = useCallback(() => {
    if (
      decodedData &&
      (decodedData.startsWith('http://') || decodedData.startsWith('https://'))
    ) {
      ...
      >
        <View style={styles.container}>
          ...
          <View style={styles.section}>
            ...
            >
              Downloads only the image URL you enter. Gallery and clipboard
          ...
          >
            ...
            <Text style={[styles.infoText, { color: colors.text }]}>
              QR generation and local image decoding happen on your device.
              Remote image decoding fetches only the image URL you enter.
            </Text>
          </View>
        </View>
      </ScrollView>
